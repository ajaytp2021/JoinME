...
<?php
        ...
        <ul class="list-group pl-3 card card-body col-xl-12 col-lg-12 col-md-12 mb-3 hvr-overline-reveal">
            ...
            <h5 class="float-right text-right mr-5 text-secondary">ID:&nbsp;<?php echo $row[0]; ?></h5>
              <a href="<?php echo $host; ?>/joinme/Admin/viewapprovedcompanies?rawvalue=<?php echo $val; ?>&id=<?php echo $row[0]; ?>" class="float-right text-right"><img src="../assets/images/icons/visibility.svg"></img></a>
            </li>
          </ul>
          <?php
          }
...
<nav aria-label="...">
  ...
  <li class="page-item <?php if($page == 1){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($page > 1){ ?> href="approvedcompanies?rawvalue=<?php echo $val; ?>&page=<?php echo pageBack($page); ?>" <?php } ?>>Previous</a>
    </li>
    <?php
for($pcount = 1; $pcount <= $currentpagecount; $pcount++){
?>
    <li class="page-item <?php if($pcount == $page){ echo 'active'; } ?>"><a class="page-link" <?php if($pcount != $page){ ?> href="approvedcompanies?rawvalue=<?php echo $val; ?>&page=<?php echo $pcount; ?>" <?php } ?>><?php echo $pcount; ?></a></li>

<?php } ?>
    <li class="page-item <?php if($currentpagecount == $page){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($currentpagecount != $page){ ?> href="approvedcompanies?rawvalue=<?php echo $val; ?>&page=<?php echo pageForward($page); ?>" <?php } ?>>Next</a>
    </li>
  </ul>
</nav>
<?php } ?>
