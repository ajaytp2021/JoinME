...
<body class="w-100 h-100">
  ...
  <ol class="breadcrumb">
    <li class="breadcrumb-item" aria-current="page"><a href="currentwork?rawvalue=<?php echo $val; ?>" class="text-decoration-none">MY CURRENT WORK</a></li>
...
<ul class="list-group pl-3 card card-body border-success col-xl-12 col-lg-12 col-md-12 mb-3">
            ...
            <small class="text-secondary w-100 text-left col-xl-12 col-lg-12 col-md-12 text-uppercase"><?php echo "Salary : Rs.".$row['salary']; ?></small>
            </div>
            </div>
...
<nav aria-label="...">
  ...
  <li class="page-item <?php if($page == 1){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($page > 1){ ?> href="completedprojects?rawvalue=<?php echo $val; ?>&page=<?php echo pageBack($page); ?>" <?php } ?>>Previous</a>
    </li>
    <?php
for($pcount = 1; $pcount <= $currentpagecount; $pcount++){
?>
    <li class="page-item <?php if($pcount == $page){ echo 'active'; } ?>"><a class="page-link" <?php if($pcount != $page){ ?> href="completedprojects?rawvalue=<?php echo $val; ?>&page=<?php echo $pcount; ?>" <?php } ?>><?php echo $pcount; ?></a></li>

<?php } ?>
    <li class="page-item <?php if($currentpagecount == $page){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($currentpagecount != $page){ ?> href="completedprojects?rawvalue=<?php echo $val; ?>&page=<?php echo pageForward($page); ?>" <?php } ?>>Next</a>
    </li>
  </ul>
</nav>
          <?php }else{ ?>
