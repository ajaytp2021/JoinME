...
if(!isset($_SESSION["loginuser"]) && empty($_SESSION["loginuser"])){
    ...
    if($_SESSION["loginuser"] != $val){
        ...
        if($con){
          $skills = "select skills.*, userSkill.levels from userSkill inner join skills on userSkill.SKId=skills.SKId where userSkill.UId=".base64_decode($id);
...
<body class="w-100 h-100">
  ...
  <option value="novalue" selected disabled>Choose skill level</option>
...
<input type="submit" class="btn btn-danger" name="addskillbtn" value="Add Skill" onclick="return confirm('Do you want to add this skill?')">
        ...
        <ul class="list-group pl-3 card card-body col-xl-12 col-lg-12 col-md-12 mb-3 hvr-overline-reveal">
            <li class="d-flex align-content-center">
              ...
              <h6 class="text-secondary font-weight-bold w-100 text-left"><?php echo $row[1]; ?></h6>
              <h6 class="text-secondary text-left"><?php echo $row[2]; ?></h6>
              </div>
              <a href="<?php echo $host; ?>/joinme/php/deleteskilluser?rawvalue=<?php echo $val; ?>&id=<?php echo $id; ?>&skid=<?php echo base64_encode($row[0]); ?>" class="float-right text-right" onclick="return confirm('Do you want to delete <?php echo $row[1]; ?>?')"><img src="../assets/images/icons/delete.svg"></img></a>
            </li>
          </ul>
          <?php
          }
...
$level = $_POST['level'];
$insert = "insert into userSkill values ($skillid, ".base64_decode($id).", '$level')";
