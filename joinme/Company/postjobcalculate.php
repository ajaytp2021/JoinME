...
if(!isset($_SESSION["logincompany"]) && empty($_SESSION["logincompany"])){
    ...
    if($_SESSION["logincompany"] != $val){
        ...
        if($con){
          ...
          $jTitle = [];
          $salaries = [];
          $selectSkill = "select * from companySalaryScale where jTitle='$skill' and CId=".base64_decode($cid);
          if($res = mysqli_query($con, $selectSkill)){
            while($row = mysqli_fetch_array($res)){
              $levels = array_merge($levels, array($row['levels']));
              $jTitle = array_merge($jTitle, array($row['jTitle']));
              $salaries = array_merge($salaries, array($row['salary']));
            }
...
for($i = 0; $i < count($levels); $i++){
  ...
  if($x != 0){
?>
<form action="../php/postjob" method="POST" class="w-100 col-xl-6 col-lg-6 col-md-6 pr-3 pl-5 pb-3">
  ...
  <?php
   ...
   ?>
            ...
            <a href="../php/postjob?rawvalue=<?php echo $val."&title=".base64_encode($ptitle)."&desc=".base64_encode($desc)."&edate=".base64_encode($pexpire)."&nousr=".base64_encode($x)."&ulevel=".base64_encode($lvl[$i])."&jtitle=".base64_encode($skill)."&salary=".base64_encode($salaries[$i])."&texp=".base64_encode($tExp)."&months=".$month; ?>" class="btn btn-success w-100 pulsebtn" name="postjob">POST JOB</a>
            </div>
  </form>
  </div>
<?php
