...
<?php
      ...
      <?php } ?>
        ...
        <?php if($row['ratings'] != null){ ?><div class="text-white text-center rounded font-weight-bold pl-3 pr-3 float-right align-self-center" style="height: fit-content; width: fit-content; background-color: <?php if($row['ratings'] >= 3){ echo '#4CAF50'; }else if(3 > $row['ratings'] && $row['ratings'] > 1){ echo '#ff9800'; }else{ echo '#f44336'; } ?>"><?php echo $row['ratings']; ?></div><?php }else{ ?><p class="small card align-self-center font-weight-bold text-secondary pl-3 pr-3 m-0" style="width: fit-content;">NILL</p><?php } ?>
              <a href="<?php echo $host; ?>/joinme/Company/userdetailedview?rawvalue=<?php echo $val; ?>&uid=<?php echo base64_encode($row['UId']); ?>" class="float-right col-xl-12 col-lg-12 col-md-12 m-0 p-0"><img src="../assets/images/icons/visibility.svg"></img></a>
              <?php if($isstart){ ?><a href="viewworkreports?rawvalue=<?php echo $val; ?>&uid=<?php echo base64_encode($row['UId']); ?>&pid=<?php echo base64_encode($postId); ?>&uname=<?php echo base64_encode($row['name']); ?>&ptitle=<?php echo base64_encode($ptitle); ?>" class="text-decoration-none small">View reports</a><?php } ?>
              </div>
            </li>
          </ul>
...
<nav aria-label="...">
  ...
  <li class="page-item <?php if($page == 1){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($page > 1){ ?> href="uop?rawvalue=<?php echo $val; ?>&ptitle=<?php echo $ptitle.'&pid='.base64_encode($postId); ?>&page=<?php echo pageBack($page); ?>" <?php } ?>>Previous</a>
    </li>
    <?php
for($pcount = 1; $pcount <= $currentpagecount; $pcount++){
?>
    <li class="page-item <?php if($pcount == $page){ echo 'active'; } ?>"><a class="page-link" <?php if($pcount != $page){ ?> href="uop?rawvalue=<?php echo $val; ?>&ptitle=<?php echo $ptitle.'&pid='.base64_encode($postId); ?>&page=<?php echo $pcount; ?>" <?php } ?>><?php echo $pcount; ?></a></li>

<?php } ?>
    <li class="page-item <?php if($currentpagecount == $page){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($currentpagecount != $page){ ?> href="uop?rawvalue=<?php echo $val; ?>&ptitle=<?php echo $ptitle.'&pid='.base64_encode($postId); ?>&page=<?php echo pageForward($page); ?>" <?php } ?>>Next</a>
    </li>
  </ul>
</nav>
<?php } ?>
