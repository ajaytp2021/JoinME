        ...
        if($con){
          ...
          if($res = mysqli_query($con, $skills)){
            if(mysqli_num_rows($res) != 0){
              ...
              $passpost = "select DISTINCT(ps.PId) as checkid, ps.*, cm.cname from posts ps left join prjSkills prj on ps.PId=prj.PId left join company cm on ps.CId=cm.CId where prj.skill in (select skills.skill from skills inner join userSkill on skills.SKId=userSkill.SKId where userSkill.UId=".base64_decode($id).") and ps.EDate >= '".date('Y-m-d')."' ";
            }else{
              ...
              $passpost = "select DISTINCT(ps.PId) as checkid, ps.*, cm.cname from posts ps left join prjSkills prj on ps.PId=prj.PId left join company cm on ps.CId=cm.CId where ps.EDate >= '".date('Y-m-d')."' ";
            }
          }
...
if($res = mysqli_query($con, $posts)){
  ...
  if($nr != 0){
    while($row = mysqli_fetch_array($res)){ 
      ?>
<a href="postdetailed.php?rawvalue=<?php echo $val; ?>&details=<?php echo base64_encode($passpost."and ps.PId=".$row['PId']." order by ps.pDate desc, ps.EDate asc"); ?>&pid=<?php echo base64_encode($row['PId']); ?>&cid=<?php echo base64_encode($row['CId']); ?>" class="p-0 m-0 text-decoration-none col-xl-12 col-lg-12 col-md-12">
<ul class="list-group pl-3 card card-body col-xl-12 col-lg-12 col-md-12 mb-3">
            ...
            <h6 class="text-secondary text-left font-weight-bold w-100 col-xl-12 col-lg-12 col-md-12 text-uppercase"><?php echo $row['pTitle']; ?></h6>
           ...
           ?>
            ...
            <!-- <small class="text-secondary w-100 text-left col-xl-12 col-lg-12 col-md-12">Salary : Rs.<?php //echo $row['salary']; ?></small> -->
              ...
              <small class="text-secondary align-bottom h-auto w-100 float-right col-xl-12 col-lg-12 col-md-12 m-0 p-0 align-bottom h-100 align-text-bottom">End Date : <?php echo date_format(date_create($row['EDate']), "d-M-Y"); ?></small>
             </div>
            </li>
          </ul>
          </a>
...
<nav aria-label="...">
  ...
  <li class="page-item <?php if($page == 1){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($page > 1){ ?> href="home.php?rawvalue=<?php echo $val; ?>&page=<?php echo pageBack($page); ?>" <?php } ?>>Previous</a>
    </li>
    <?php
for($pcount = 1; $pcount <= $currentpagecount; $pcount++){
?>
    <li class="page-item <?php if($pcount == $page){ echo 'active'; } ?>"><a class="page-link" <?php if($pcount != $page){ ?> href="home.php?rawvalue=<?php echo $val; ?>&page=<?php echo $pcount; ?>" <?php } ?>><?php echo $pcount; ?></a></li>

<?php } ?>
    <li class="page-item <?php if($currentpagecount == $page){ echo 'disabled'; } ?>">
      <a class="page-link" <?php if($currentpagecount != $page){ ?> href="home.php?rawvalue=<?php echo $val; ?>&page=<?php echo pageForward($page); ?>" <?php } ?>>Next</a>
    </li>
  </ul>
</nav>
<?php } ?>
